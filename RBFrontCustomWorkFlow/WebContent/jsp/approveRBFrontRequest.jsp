...
<%
		...
		*/
		...
		if (debugging) System.out.println("approveRBFrontRequest: Data check sDate=" + sDate + "- eDate=" + eDate + "- type="+ type + "- complete=" + complete);
		...
		if(sDate != null || eDate != null || type != null || complete != null ){
			...
			if (request.getParameter("attrib") != null) {
				if (request.getParameter("attrib").equals("update")) {
					if (debugging) System.out.println("approveRBFrontRequest.jsp: Back from UPDATE function");
					if (debugging) System.out.println("approveRBFrontRequest.jsp: sDate=" + request.getParameter("EffStartDateS"));
					if (debugging) System.out.println("approveRBFrontRequest.jsp: eDate=" + request.getParameter("EffStartDateE"));
				} else {
					if (debugging) System.out.println("approveRBFrontRequest.jsp: Found attrib parameter but it is not UPDATE");
				}
			} else {
...
//		old code comment								  	out.println("<td class=\"tbl_row1\" ><input "+checkRed2(resultSet.getString("NewTermID"))+" class=\"txt1\" name=\"NewTermID"+i+"\" type=\"text\" value=\""+resultSet.getString("NewTermID")+"\" size=\"10\"  /></td>");
										// new code start - add new String "outmode" & "rem" & "newterm" var
										  	System.out.flush();										  	
											newterm = (!"".equals(resultSet.getString("NewTermID"))) ?  resultSet.getString("NewTermID").trim() : newterm;
										  	out.println("<td class=\"tbl_row1\" ><input " + checkRed2(newterm) + " class=\"txt1\" name=\"NewTermID"+i+"\" type=\"text\" value=\""+newterm+"\" size=\"10\"  /></td>");
